...
const FAQ_ES = [
  ...
  { q: '¿Qué documentos y costos tiene una compraventa?', a: `En general intervienen el título de propiedad, la cédula o identificación de las partes, los certificados y comprobantes de impuestos y servicios al día, y la escritura ante ${esc}. A eso se suman los honorarios y los impuestos de transferencia que correspondan. Los montos y requisitos cambian según el caso y el país, así que conviene confirmarlos con tu ${esc} antes de firmar.` },
  ...
  { q: '¿En qué ciudades hay propiedades?', a: `Casa Libre reúne propiedades de todo ${COUNTRY.name}: ${CITIES.map((x) => x.name).slice(0, 8).join(', ')} y más.` },
...
const FAQ_EN = [
  ...
  { q: 'Which cities have properties?', a: `Casa Libre gathers properties from all over ${COUNTRY.name}: ${CITIES.map((x) => x.name).slice(0, 8).join(', ')} and more.` },
