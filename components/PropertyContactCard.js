'use client';
// Property detail WhatsApp contact card (V2 brand). White card with ink border +
// hard offset shadow, seller header, buyer-name input, editable message preview,
// green WhatsApp CTA, call/copy actions, and a safety note. Bilingual (ES/ES via
// useLang). Fires contact_seller_clicked (PostHog) on WhatsApp tap.
...
const T = {
  es: {
    owner: 'El propietario', role: 'Contacto directo',
    nameLbl: 'Tu nombre (opcional)', namePh: 'Ej: Carlos',
    msgLbl: 'Tu mensaje', msgHint: 'Se abre en WhatsApp — podés editarlo antes de enviar.',
    ...
    msg: (name, url) => `¡Hola! Vi esta propiedad en Casa Libre y me interesa. ¿Sigue disponible?\n${url}`,
  },
  en: {
    owner: 'The owner', role: 'Direct contact',
    nameLbl: 'Your name (optional)', namePh: 'E.g. Carlos',
    msgLbl: 'Your message', msgHint: 'Opens in WhatsApp — you can edit it before sending.',
    ...
    msg: (name, url) => `Hi${name ? `, I'm ${name.trim()}` : ''}! Is this property still available?\n${url}`,
  },
};
...
export default function PropertyContactCard({ sellerName, waDigits, url, trackProps }) {
  const [lang] = useLang();
  ...
  // The message reaches a local (Paraguayan) seller, so it is always in Spanish
  // regardless of the buyer's UI language.
      ...
      <div className="bg-card overflow-hidden" style={{ border: '1.5px solid #111', borderRadius: 18, boxShadow: '5px 4px 0 #111' }}>
        ...
        <div className="flex items-center gap-3 px-[18px] py-4 border-b border-ink/12">
          ...
          <div className="min-w-0">
            <b className="block text-[15px] font-semibold truncate">{displayName}</b>
          </div>
        </div>
          ...
          {waUrl ? (
            <>
              <a
                ...
                onClick={() => track('contact_seller_clicked', { ...(trackProps || {}), channel: 'whatsapp' })}
                className="w-full flex items-center justify-center gap-2.5 px-4 py-3 rounded-pill text-[15px] font-semibold text-white transition-transform active:translate-x-[2px] active:translate-y-[2px]"
