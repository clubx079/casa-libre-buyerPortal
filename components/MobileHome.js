...
const H = {
  es: {
    ...
    heroSub: [`Todas las propiedades de ${COUNTRY.name}`, 'en una sola app: inmobiliarias', 'y dueños particulares, sin comisión.'],
  ...
  en: {
    ...
    heroSub: [`Every property in ${COUNTRY.name} in one app —`, 'agencies and private owners,', 'with no commission.'],
...
export default function MobileHome({ featured = [], count = 0, tickerData = [] }) {
  ...
  const btnCls = (k) => `flex-1 text-center py-[13px] rounded-pill border-[1.5px] border-ink text-[15px] font-bold transition-colors ${sel === k ? 'bg-ink text-paper' : 'bg-card text-ink'}`;
  ...
  return (
    <div className="bg-paper">
      ...
      {/* BUY / RENT / SELL — three distinct buttons */}
