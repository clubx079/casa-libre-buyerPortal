...
// A property counts as currently highlighted only while its paid window is live.
const isHi = (r) => !!(r.is_highlighted && r.highlighted_until && new Date(r.highlighted_until) > new Date());
...
export async function searchListings(p = {}) {
  ...
  parts.push('select=id,latitude,longitude,listing_type,price,currency,property_type,city,neighborhood,address,province,bedrooms,bathrooms,parking_spaces,covered_area,floor_area,land_area,source_id,is_highlighted,highlighted_until');
  ...
  // Paid highlights always float to the top, regardless of the chosen sort.
  parts.push(`order=is_highlighted.desc,${primary}`);
...
export async function getPins(p = {}) {
  ...
  parts.push('select=id,latitude,longitude,listing_type,price,currency,is_highlighted,highlighted_until', 'order=is_highlighted.desc,created_at.desc', `limit=${PIN_LIMIT}`);
  let rows = [];
  try { rows = await select('properties', parts.join('&')); } catch { rows = []; }
  const rate = await getUsdToPyg();
  return (rows || []).map((r) => {
    const m = dualPrice(r.price, r.currency, rate);
    return { id: r.id, lat: Number(Number(r.latitude).toFixed(5)), lng: Number(Number(r.longitude).toFixed(5)), usd: m.usd, mode: r.listing_type === 'rent' ? 'alquiler' : 'venta', hl: isHi(r) };
  });
}
