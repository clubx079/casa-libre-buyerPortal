...
// Every email (codes included) goes out from the ACTIVE country's own domain —
// fixed in code, not taken from RESEND_FROM, so a stale env value (it once said
// notifications@deelmap.com) can never send Casa Libre mail from another brand.
const SENDER_DOMAIN = {
  py: 'casa-libre.com.py',
  bo: 'casa-libre.com.bo',
  uy: 'uy.casa-libre.com',
  ve: 'casa-libre.com.ve',     // needs its domain verified in Resend before it can send
};
const FROM = `${COUNTRY.brand} <no-reply@${SENDER_DOMAIN[COUNTRY.code] || SENDER_DOMAIN.py}>`;
...
// ── Admin-editable templates (email_templates) ──────────────────────────────────
// The frame lib/emailTemplateRender.js draws around a template: same header, card and
// app-badge footer as every other Casa Libre email (badges ride along as cid images).
export function templateFrame() {
  return {
    brand: COUNTRY.brand, tld: COUNTRY.tld, countryName: COUNTRY.name,
    appleSrc: `cid:${BADGE_APPLE_CID}`, playSrc: `cid:${BADGE_PLAY_CID}`,
    downloadUrl: `${SITE}/descargar`,
  };
}

export const siteUrl = () => SITE;

// Send an already-rendered email ({ subject, html, text }). Never throws.
// Returns { ok, id } or { ok:false, error }.
export async function sendRenderedEmail({ to, subject, html, text }) {
  if (!resend) return { ok: false, error: 'email_not_configured' };
  if (!to) return { ok: false, error: 'no_recipient' };
  try {
    const { data, error } = await withTimeout(
      resend.emails.send({ from: FROM, attachments: badgeAttachments(), to, subject, html, text }),
      15000
    );
    if (error) return { ok: false, error: error.message || 'send_failed' };
