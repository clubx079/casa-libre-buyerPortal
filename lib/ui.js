...
// Per-country values (country name, locale, currency symbol, phone prefix) come
// from lib/country.js (NEXT_PUBLIC_COUNTRY); Paraguay is unchanged.
...
export const T = {
  es: {
    ...
    heroTitle1: 'Tu próximo hogar', heroTitle2: `en ${COUNTRY.name}`,
  ...
  en: {
    ...
    heroTitle1: 'Your next home', heroTitle2: `in ${COUNTRY.name}`,
    heroSub: 'Houses, apartments and more — buy and rent with Casa Libre.',
    heroCta: 'Browse properties', featured: 'Featured',
  },
};

export const loc = (lang) => (lang === 'en' ? 'en-US' : COUNTRY.locale);
export const fmtUsd = (v, lang) => (v == null ? null : 'US$ ' + Number(v).toLocaleString(loc(lang)));
export const fmtPyg = (v, lang) => (v == null ? null : COUNTRY.currencySymbol + ' ' + Number(v).toLocaleString(loc(lang)));
...
export const titleCaseZone = (s) => {
  if (!s) return '';
  return String(s)
    .toLocaleLowerCase(COUNTRY.locale)
    .replace(/\b([\p{L}])([\p{L}]*)/gu, (_, a, b) => a.toLocaleUpperCase(COUNTRY.locale) + b)
    .trim();
};
...
// --- Phone normalize to wa.me digits (country dialing code from lib/country.js) ---
export const normalizePhone = (phone) => {
  const d = String(phone ?? '').replace(/\D/g, '');
  if (!d) return '';
  const p = COUNTRY.phonePrefix;
  if (d.startsWith(p)) return d;
  if (d.startsWith('0')) return p + d.slice(1);
  return p + d;
};
// Back-compat alias (existing imports use normalizePy).
