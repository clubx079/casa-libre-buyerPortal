...
const PROFILES = {
  ...
  py: {
    ...
    desc: 'Casa Libre es el marketplace de propiedades en Paraguay. Comprá, alquilá y publicá casas, departamentos, dúplex y locales en Asunción y todo el país — gratis.',
  ...
  bo: {
    ...
    desc: 'Casa Libre es el marketplace de propiedades en Bolivia. Comprá, alquilá y publicá casas, departamentos, dúplex y locales en Santa Cruz y todo el país — gratis.',
  ...
  uy: {
    ...
    brand: 'Casa Libre',
    tld: '.uy',
    defaultUrl: 'https://casa-libre.com.uy',
    capital: 'Montevideo',
    ...
    desc: 'Casa Libre es el marketplace de propiedades en Uruguay. Comprá, alquilá y publicá casas, departamentos, dúplex y locales en Montevideo y todo el país — gratis.',
...
function resolveCountryCode() {
  if (typeof window !== 'undefined') {
    // Client: trust the value the server stamped into the page (from runtime COUNTRY).
    return String(window.__CL_COUNTRY__ || 'py').toLowerCase();
  }
  // Server: prefer the PLAIN runtime var COUNTRY (read live from the container env at
  // request time — set it once per Coolify service and a redeploy/restart can't lose
  // it). NEXT_PUBLIC_COUNTRY is only a fallback: Next INLINES it at build, so it is
  // frozen into the image and cannot change at runtime.
  return String(
    process.env.COUNTRY
