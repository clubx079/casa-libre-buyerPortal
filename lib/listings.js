...
const LIGHT_SELECT = [
  'id,slug,address,city,neighborhood,province,price,currency,listing_type,property_type',
  'bedrooms,bathrooms,floor_area,covered_area,land_area,parking_spaces,latitude,longitude',
  'feature_image_url,created_at,source_id,contact_phone',
].join(',');
...
export async function getListings({ limit = 400, light = false } = {}) {
  ...
  return { rate, rateSource: fx.source, rateDate: fx.updated, count: complete.length, listings: complete };
}
...
export async function getActiveCount() {
  const parts = [
    'select=price,currency,listing_type,bedrooms,bathrooms,parking_spaces,covered_area,floor_area,land_area,city,neighborhood,property_type,contact_phone',
    'admin_status=eq.active',
    'contact_phone=not.is.null',
    'price=gt.0',
    'limit=8000',
  ...
  return rows.map((r) => shape(r, rate)).filter(isCompleteListing).length;
}
