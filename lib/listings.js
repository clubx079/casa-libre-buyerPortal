...
// The per-property short_code (for the short WhatsApp /s/<code> link) currently
// exists only on the Paraguay DB. Selecting a missing column 400s the whole
// fetch, so include it ONLY for PY; BO/UY fall back to the UUID-embed link until
// their DBs get the column too.
const HAS_SHORT_CODE = COUNTRY.code === 'py';
const SELECT = [
  ...
  'description,features,feature_image_url,external_url,contact_name,contact_phone,created_at,first_scraped_at,source_id,promotion_plan,promotion_expires_at' + (HAS_SHORT_CODE ? ',short_code' : ''),
...
const LIGHT_SELECT = [
  'id,slug,address,city,neighborhood,province,price,currency,listing_type,property_type',
  'bedrooms,bathrooms,floor_area,covered_area,land_area,parking_spaces,latitude,longitude',
  'created_at,source_id,contact_phone,promotion_plan,promotion_expires_at',
].join(',');
...
function shape(r, rate) {
  ...
  return {
    ...
    // When WE first scraped/listed it into the Casa Libre ecosystem — the basis
    // for the "N days on Casa Libre" badge. created_at is unreliable for some
    // sources (e.g. Habitamia carries the source's original date, 2015-01-01).
    first_scraped_at: r.first_scraped_at || r.created_at || null,
    ...
    short_code: r.short_code || null,   // powers the short WhatsApp /s/<code> link (PY)
    ...
    // Paid promotion — live only while the 30-day window is open. Both plans are
    // "verified"; only the 'home' plan also shows on the landing page.
    ...promoFields(r),
  };
}

// Derive the client-facing promotion flags from the raw promotion_* columns.
function promoFields(r) {
  const promoted = !!(r.promotion_plan && r.promotion_expires_at && new Date(r.promotion_expires_at) > new Date());
  const plan = promoted ? r.promotion_plan : null;
  return {
    verified: promoted,                 // both 'verified' and 'home' show the Verified badge
    plan,                               // 'verified' | 'home' | null
    onHome: plan === 'home',            // only the landing plan appears on the home page
    highlighted: promoted,              // legacy alias (ring styling); kept so nothing breaks
    promotion_expires_at: promoted ? r.promotion_expires_at : null,
  };
}
...
export async function getListings({ limit = 400, light = false, withImage = false, mode } = {}) {
  const parts = [
    // withImage adds just the card cover url to the light projection (mobile slim feed).
    `select=${light ? LIGHT_SELECT + (withImage ? ',feature_image_url' : '') : SELECT}`,
    ...
    'price=gt.0',
    // Optional operation filter — when set, BOTH the fetched rows and the exact
    // count (count=exact) reflect just that mode, so totalCount is per-mode
    // accurate. Default (undefined) = no filter → all existing callers unchanged.
    ...(mode === 'venta' ? ['listing_type=neq.rent'] : mode === 'alquiler' ? ['listing_type=eq.rent'] : []),
    // Normal order (newest first). Promotions no longer float to the top — a
    // "verified" listing keeps its natural relevance position; the paid value is the
    // Verified badge + the map star (and, for 'home', a spot on the landing page).
...
const slimForMarketplace = (l) => ({
  ...
  highlighted: l.highlighted, verified: l.verified, plan: l.plan, onHome: l.onHome,
});
...
// Mobile app browse/map set (slim feed). The app loads the WHOLE catalog once and
// filters it on-device to the visible map area, so this ships only card/map/filter
// fields + the cover url — never contact_phone/contact_name, description, features,
// the image array or external_url (those stay behind /api/mobile/listing/[id]).
// That keeps the anti-scraping hardening intact: the full-detail feed stays at 600.
...
// Listings currently on a FREE first-listing gift (automation_runs gifted/reminded).
// The home strip shows paid Landing listings first and these only in leftover slots.
// Before migration 005 is applied the table doesn't exist → no free listings.
export async function getFreeHomeIdsRaw() {
  try {
    const rows = await select('automation_runs', 'select=property_id&status=in.(gifted,reminded)&limit=1000');
    return (rows || []).map((r) => String(r.property_id)).filter(Boolean);
  } catch { return []; }
}
export const getFreeHomeIds = unstable_cache(getFreeHomeIdsRaw, ['cl-free-home-ids-v1'], { revalidate: 300, tags: ['listings'] });
...
// All listings a user has published (any status), newest first. Carries the
// promotion state so the dashboard can show a Verified badge / days-left / renew.
export async function getUserListings(userId) {
  ...
  const listings = rows.map((r) => ({
    ...
    created_at: r.created_at || null,
    // shape() already yields verified/plan/onHome/highlighted/promotion_expires_at.
  }));
  return { rate, listings };
}
