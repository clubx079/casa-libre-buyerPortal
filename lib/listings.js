...
export async function getListings({ limit = 400 } = {}) {
  ...
  // `total` is the exact DB count of active, contactable, priced buildings
  // (ignores the fetch limit) — used for the homepage "active listings" stat so
  // it reflects the real catalogue, not the limited page.
  return { rate, rateSource: fx.source, rateDate: fx.updated, count: complete.length, total: count, listings: complete };
}
