...
const SELECT = [
  ...
  'description,features,feature_image_url,external_url,contact_name,contact_phone,created_at,first_scraped_at,source_id,is_highlighted,highlighted_until',
...
const LIGHT_SELECT = [
  'id,slug,address,city,neighborhood,province,price,currency,listing_type,property_type',
  'bedrooms,bathrooms,floor_area,covered_area,land_area,parking_spaces,latitude,longitude',
  'created_at,source_id,contact_phone,is_highlighted,highlighted_until',
].join(',');
...
function shape(r, rate) {
  ...
  return {
    ...
    // Paid highlight — live only while the 30-day window is open.
    highlighted: !!(r.is_highlighted && r.highlighted_until && new Date(r.highlighted_until) > new Date()),
  };
}
...
export async function getListings({ limit = 400, light = false } = {}) {
  const parts = [
    ...
    'price=gt.0',
    // Paid highlights float to the top of the marketplace + home lists.
    'order=is_highlighted.desc,created_at.desc',
    `limit=${limit}`,
    ...buildingsParts(),
  ];
...
const slimForMarketplace = (l) => ({
  ...
  area: l.area, user_published: l.user_published, highlighted: l.highlighted,
});
...
// highlight state so the dashboard can show a "Destacada" badge / days-left.
export async function getUserListings(userId) {
  const rate = await getUsdToPyg();
  if (!userId) return { rate, listings: [] };
  const parts = [`select=${SELECT},admin_status,created_at,is_highlighted,highlighted_until`, `created_by=eq.${encodeURIComponent(userId)}`, 'order=created_at.desc', 'limit=300'];
  let rows = [];
  try { rows = await select('properties', parts.join('&')); } catch { rows = []; }
  const now = Date.now();
  const listings = rows.map((r) => ({
    ...
    created_at: r.created_at || null,
    highlighted: !!(r.is_highlighted && r.highlighted_until && new Date(r.highlighted_until).getTime() > now),
    highlighted_until: r.highlighted_until || null,
  }));
  return { rate, listings };
}
