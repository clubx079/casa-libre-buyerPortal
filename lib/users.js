...
export async function findOrCreateGoogleUser({ email, googleId, fullName }) {
  const e = norm(email);
  const now = new Date().toISOString();
  const existing = await findUserByEmail(e);
  if (existing) {
    const res = await update('users', `id=eq.${existing.id}`,
      { google_id: googleId, auth_provider: 'google', verified: true, full_name: existing.full_name || fullName || null, last_login_at: now },
      { returning: 'representation' });
    return (Array.isArray(res) ? res[0] : res) || existing;
  }
  const res = await insert('users',
    { email: e, google_id: googleId, auth_provider: 'google', full_name: fullName || null, verified: true, active: true, last_login_at: now },
    { returning: 'representation' });
  return Array.isArray(res) ? res[0] : res;
}
...
export async function touchLogin(id) {
  try { await update('users', `id=eq.${encodeURIComponent(id)}`, { last_login_at: new Date().toISOString() }, { returning: 'minimal' }); } catch {}
}
