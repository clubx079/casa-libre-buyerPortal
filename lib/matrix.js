...
// `label`/`short`/`plural` are the Spanish strings the pages are written in (the
// SEO target); the *En variants feed the site's EN toggle on those same URLs.
export const OPS = {
  venta: { op: 'venta', label: 'en venta', short: 'Venta', labelEn: 'for sale', shortEn: 'For sale' },
  alquiler: { op: 'alquiler', label: 'en alquiler', short: 'Alquiler', labelEn: 'for rent', shortEn: 'For rent' },
};
...
export const TIPOS = {
  casas: { type: 'casa', kw: ['casa', 'house'], sing: 'casa', plural: 'casas', singEn: 'house', pluralEn: 'houses' },
  departamentos: { type: 'depto', kw: ['departamento', 'depto', 'apartment', 'flat'], sing: 'departamento', plural: 'departamentos', singEn: 'apartment', pluralEn: 'apartments' },
  duplex: { type: 'duplex', kw: ['duplex', 'dúplex'], sing: 'dúplex', plural: 'dúplex', singEn: 'duplex', pluralEn: 'duplexes' },
  oficinas: { type: 'oficina', kw: ['oficina', 'office'], sing: 'oficina', plural: 'oficinas', singEn: 'office', pluralEn: 'offices' },
  locales: { type: 'comercial', kw: ['comercial', 'local', 'commercial', 'tienda'], sing: 'local comercial', plural: 'locales comerciales', singEn: 'commercial unit', pluralEn: 'commercial units' },
  depositos: { type: 'deposito', kw: ['deposito', 'depósito', 'warehouse', 'galp'], sing: 'depósito', plural: 'depósitos', singEn: 'warehouse', pluralEn: 'warehouses' },
};
...
export function comboContentBarrio({ op, tipo, barrioName, cityName, count = 0 }) {
  ...
  const title = `${Tipo} ${o.label} en ${place} — ${count} propiedades`;
  const description = `${count} ${t.plural} ${o.label} en ${barrioName}, ${cityName}, de inmobiliarias y de dueños particulares. Explorá precios, fotos y ubicación en el mapa, en la web o en la app. Buscar es gratis.`;
  const intro = `Encontrá ${t.plural} ${o.label} en ${barrioName} (${cityName}). Casa Libre reúne en un solo lugar ${count} ${count === 1 ? 'propiedad' : 'propiedades'} de ${t.plural} ${o.label} en ${barrioName} y la zona, publicadas por inmobiliarias y por dueños particulares — filtrá por precio, dormitorios y superficie, y compará cada opción en el mapa con fotos reales, desde la web o la app.`;
  return { h1, title, description, intro, place };
}

// English copy for the same combo — used by the ES/EN toggle on the page body.
// (Metadata stays Spanish: that is what crawlers index.)
export function comboContentEn({ op, tipo, ciudad, count = 0 }) {
  const o = OPS[op];
  const t = TIPOS[tipo];
  const city = CITIES.find((c) => c.slug === ciudad);
  const cityName = city ? city.name : ciudad;
  const Tipo = (t.pluralEn || t.plural).charAt(0).toUpperCase() + (t.pluralEn || t.plural).slice(1);
  const h1 = `${Tipo} ${o.labelEn} in ${cityName}`;
  const intro = `Find ${t.pluralEn} ${o.labelEn} in ${cityName}. Casa Libre brings together ${count} ${count === 1 ? 'property' : 'properties'} in and around ${cityName}, listed by real-estate agencies and by private owners — filter by price, bedrooms and floor area, and compare each one on the map with real photos, on the web or in the app.`;
  return { h1, intro, cityName };
}

export function comboContentBarrioEn({ op, tipo, barrioName, cityName, count = 0 }) {
  const o = OPS[op];
  const t = TIPOS[tipo];
  const Tipo = (t.pluralEn || t.plural).charAt(0).toUpperCase() + (t.pluralEn || t.plural).slice(1);
  const h1 = `${Tipo} ${o.labelEn} in ${barrioName}, ${cityName}`;
  const intro = `Find ${t.pluralEn} ${o.labelEn} in ${barrioName} (${cityName}). Casa Libre brings together ${count} ${count === 1 ? 'property' : 'properties'} in ${barrioName} and nearby, listed by real-estate agencies and by private owners — filter by price, bedrooms and floor area, and compare each one on the map with real photos, on the web or in the app.`;
  return { h1, intro };
}
...
export function comboContent({ op, tipo, ciudad, count = 0 }) {
  ...
  const title = `${Tipo} ${o.label} en ${cityName} — ${count} propiedades`;
  const description = `${count} ${t.plural} ${o.label} en ${cityName}, ${COUNTRY.name}, de inmobiliarias y de dueños particulares. Explorá precios, fotos y ubicación en el mapa, en la web o en la app. Buscar es gratis.`;
  const intro = `Encontrá ${t.plural} ${o.label} en ${cityName}. Casa Libre reúne en un solo lugar ${count} ${count === 1 ? 'propiedad' : 'propiedades'} de ${t.plural} ${o.label} en ${cityName} y alrededores, publicadas por inmobiliarias y por dueños particulares — filtrá por precio, dormitorios y superficie, y compará cada opción en el mapa con fotos reales, desde la web o la app.`;
