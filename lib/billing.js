...
import { stripe, PROMO_DAYS, promoPlan } from './stripe';
...
// Grant / extend a 30-day promotion on a property under a plan ('verified'|'home').
// For a renewal, pass { extendFrom: currentExpiryISO } so days stack onto whatever
// time is left instead of being lost. Returns the new expiry (ISO). Writes ONLY the
// promotion_* columns (never the legacy is_highlighted), so the deployed prod site —
// which still keys off is_highlighted — is unaffected by these writes.
// days: defaults to the paid 30-day window; the first-listing automation passes its own.
export async function grantPromotion(propertyId, plan, { extendFrom, days = PROMO_DAYS } = {}) {
  const now = new Date();
  const from = extendFrom && new Date(extendFrom) > now ? new Date(extendFrom) : now;
  const until = new Date(from.getTime() + days * 24 * 60 * 60 * 1000);
  await update(
    'properties',
    `id=eq.${encodeURIComponent(propertyId)}`,
    { promotion_plan: promoPlan(plan), promoted_at: now.toISOString(), promotion_expires_at: until.toISOString(), renewal_reminded_at: null },
    { returning: 'minimal' }
  );
  return until.toISOString();
}

// Back-compat: the old single-tier highlight == the new "verified" plan.
export async function grantHighlight(propertyId) {
  return grantPromotion(propertyId, 'verified');
}
...
export async function getUserBilling(userId) {
  ...
  const cols = 'id,property_id,plan,kind,amount_usd,currency,status,failure_reason,highlight_until,created_at';
